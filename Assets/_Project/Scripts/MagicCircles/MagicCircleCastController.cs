...
namespace DuckovProto.MagicCircles
{
    ...
    public sealed class MagicCircleCastController : MonoBehaviour
    {
        [Header("References")]
        ...
        private string lastMagicCircleName = "None";
        ...
        public string LastMagicCircleName => lastMagicCircleName;
        ...
        public bool TryCastSpell(SpellDefinitionSO spell, CastContext ctx, string magicCircleDisplayName, out string failReason)
        {
            ResolveRefsIfMissing();

            if (!string.IsNullOrWhiteSpace(magicCircleDisplayName))
            {
                lastMagicCircleName = magicCircleDisplayName;
            }
        ...
        private void ResolveRefsIfMissing()
        {
            ...
            if (mana == null)
            {
                ...
                if (mana == null)
                {
                    PlayerRuntimeLocator.TryGetMana(out mana);
                }
            }
        ...
        private void SetTransientStateText(string text, float duration)
        {
            ...
            if (logCastState)
            {
                Debug.Log($"[MagicCircleCast] {text}", this);
            }
        }
